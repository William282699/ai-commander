...
- **编组候选** = squad（leaderName 称呼，括号保留 id）。**未编组候选**两步分桶：
  ① **确定性空间分组，组最大直径硬上限**（Codex 第三轮：单纯连通扩张会让 A–B≤10、B–C≤10 链式滚成跨度无界的假兵力股）：合并须同时满足最近成员距离 ≤ CLUSTER_LINK_TILES(10) **且**合并后组内最大两两距离 ≤ CLUSTER_DIAMETER_MAX_TILES(20)；贪心最小链接优先，确定性平局规则；直径上限由每次合并的不变式保证；
  ② 每组贴**最近命名地点仅用于命名**，不用于判定归属。
  合成测试必须覆盖：**链式直径上限案（A–B≤10、B–C≤10、A–C 超限不得成一组；全部组直径 ≤ 上限）**、同最近地名但相距远不得合并、半径边界、单点组。
