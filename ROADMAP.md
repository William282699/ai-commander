...
**据点被降格成战线**（`FACILITY_TARGET_DOWNGRADE_BRIEF_20260810.md` + Fable 裁定
为施工合同）：长官说「支援南线前哨」，兵去了「南部战线」。**7 个实例、2 份历史记录
（撤退语义粒度账 / v3.2 结构病③）、零次修**。根因是合同缺口不是模型能力
（同分钟反证：capture 写清楚了就打得准）。修法两条在 prompt 面、判据数落点坐标、
prompt 一轮止损、修前 RED 先行。**从新 main 开工，不与语音并行。**
