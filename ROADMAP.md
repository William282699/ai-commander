...
### ▶ 第 3 级 — Command-Preflight V1：确认对话化 【当前】
大命令时：引擎纯预演算真账 → Chen 角色内顶一句（含真数字）→ 玩家自然话授权（pendingDecision 四值+缺失禁执行）。
三地基：纯预演 previewHighImpactIntent / pending 状态机（pendingId+五结果表）/ 独立 mode:"preflight" 语音通道。
提案 v2：`COMMAND_PREFLIGHT_V1_PROPOSAL.md`（按 Codex 首轮六条修订，**待二审**）。
验收核心：说"没问题，相信我，平推吧"能执行；说"不行"绝不执行。
