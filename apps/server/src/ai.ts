...
同一份 JSON 里【还必须】含根级 "spoken" 字段＝这一轮你要说给他耳朵听的那一两句。屏上的正文照旧写给眼睛（番号、精确到小数的数字、英文代号都留在正文里），spoken 是同一件事的口语版：短、像人开口说话、不念番号、不念英文名、不念小数位，也不复述他刚说的那句。它**从属于正文**——不许出现正文和单子里没有的事实，不许和它们相左；它是"说给你听的那一版"，不是第二张嘴。`;
}
