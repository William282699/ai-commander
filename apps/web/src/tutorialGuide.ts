...
export interface GuideCtx {
  ...
  /** 对话面板此刻**正被拉成单独一个窗口**。
   *  ★ 用户 2026-09-10 手测：「我试了下弹出窗口，然后他就开始说艾米莉」——
   *  玩家正在摆弄那扇窗的时候不该被推着走。同「别在玩家动手时催」那一族。 */
  panelDetached: () => boolean;
  /** 玩家把面板**拉出去过**（这一局里点过一次「弹出面板」就永远为真）。
   *  用它判「弹出→关上」这个来回真走完了没有——只判"现在关着"分不清
   *  "试过又关回来"和"压根没碰过"。 */
  playerPoppedOutPanel: () => boolean;
  /** 这一步已经开始了多少游戏秒。
...
export const GUIDE_STEPS: GuideStep[] = [
  ...
  },
  {
    ...
    say: "抬头看一眼闪着的那排数字，那是您的家底："
       ...
       + "右边那个「弹出面板」能把我们几个的对话框拉成单独一个窗口——您现在就试一下："
       + "点它拉出去看一眼。您要是有两块屏幕，把它拖到另一块上，这边整张地图就全空出来了。"
       + "看完把那个窗口关掉就行，咱们接着往下说。",
    // ★ 这句催促要**在两种状态下都说得通**：还没弹的人照着弹，弹出去愣住的人知道怎么关。
    nudge: "「弹出面板」就在顶栏右边——拉出去看一眼，看完把那个窗口关掉就行，我等着。",
    // ★ 用户 2026-09-10 原话：「让玩家弹出窗口，然后再关上，完事后，再接 emily」。
    //   原来这步是纯时间步（12 秒自动往下），结果玩家正摆弄那扇窗，陈已经开口讲艾米莉了。
    //   现在按**来回走完**判，但三个兜底都留着，免得在教学关里亲手造门槛：
    done: (c) => {
      // ① 至少读够一拍——一点就跑的话，那排数字他根本没看
      if (c.sinceStepStart < 12) return false;
      // ② 窗还开着就别打断（两屏玩家可能一直开着 ⇒ 45 秒后照样往下，不卡死）
      if (c.panelDetached()) return c.sinceStepStart >= 45;
      // ③ 弹过又关回来＝这一步真做过一遍；从没碰过也给他走（判松，§7）
      return c.playerPoppedOutPanel() || c.sinceStepStart >= 40;
    },
  },
  {
  ...
  },
  {
    ...
    say: (c) => {
      ...
      // ★ 用户 09-11 指定的示范说法：点名他真有的那个将军 + 全员压上。
      //   有第二个队长就顺带告诉他名字能点两个——但**主句只留一条**，别给菜单。
      const who = a ?? "您手上那位队长";
      const second = b ? `想再加一个人一起上，把 ${b} 的名字也说上就行。` : "";
      return "东岭上那个插着旗的敌军哨站在闪——占下它这一关就算赢，"
        + "顶上「OBJECTIVES」那个数就是记这个的，插旗的点占几个算几个。"
        + `它有人守，别一个人上。跟我说「让${who}全员进攻敌军哨站」，他手上的兵会一块儿压过去。`
        + second
        + "还能顺手指定阵型——「楔形阵冲敌军哨站」是尖头突破，「长蛇阵」是沿路走。"
        + "想把能动的都拉上去，就说「全军进攻敌军哨站」。怎么说都行，我听得懂就照办。";
    },
